...
export type RouterCustomRouterConfig = {
  extensionId: string;
  /** Judge model used to score the four tiers (optional; deterministic heuristic fallback when absent). */
  judge?: RouterModelRef;
  /** Evaluator model used to review routed responses (defaults to judge when absent). */
  evaluator?: RouterModelRef;
  /** Tier name -> provider/model candidate set used by the custom router ("tiers/candidates"). */
  tiers?: Record<string, RouterModelRef>;
  /** JitRL engine hyperparameters (defaults mirror the Python jitrl_core spec). */
  hyperparams?: JitRLHyperparamsConfig;
  /** Override the default <pilotHome>/router/jitrl-memory.json path (tests). */
  memoryPath?: string;
  /** Timeout (ms) for the post-turn evaluator request. Default 15000. */
  evalTimeoutMs?: number;
  /** Timeout (ms) for each judge scoring request. Default 15000. */
  judgeTimeoutMs?: number;
};

/**
 * JitRL hyperparameters (spec section 2, verbatim defaults):
 * k=10, beta=5.0, lam=0.05, alpha=5.0, jaccard_threshold=0.5, z_min=-10.0,
 * memory_cap=5000, seed=42, min_neighbors=1 (C1 gate; 1 == C0 semantics).
 */
export type JitRLHyperparamsConfig = {
  k?: number;
  beta?: number;
  lam?: number;
  alpha?: number;
